...
const { buildLensReport, ROUND_CHECKLISTS } = require('../src/core/lens-report');
...
describe('buildLensReport', () => {
  ...
  test('round score is 100 when all required/expected/bonus checks pass', () => {
    ...
    const allChecks = [
      ...['readme-exists', 'readme-oneliner', 'contact-team', 'audience-customer',
          'readme-demo', 'deployed-url', 'license', 'tests-or-ci', 'git-activity',
          'funding-or-roadmap', 'monetization-clarity', 'market-evidence'].map((id) => ({ id, pass: true })),
    ];
    const report = buildLensReport('pre-seed', { artifacts: allChecks, business: [] });
  ...
  test('missing items are sorted required > expected > bonus', () => {
    const checks = {
      ...
      business: [
        { id: 'monetization-clarity', pass: false },
      ],
    };
    const report = buildLensReport('pre-seed', checks);
...
describe('score (integration)', () => {
  ...
  test('BUG-1: throws on nonexistent path', () => {
    expect(() => score('/tmp/fundscore-does-not-exist-12345')).toThrow(/does not exist/);
  });
  ...
  test('fixDeltas are sorted by score impact (descending)', () => {
    ...
    if (report.fixDeltas.length > 1) {
      for (let i = 1; i < report.fixDeltas.length; i++) {
        expect(report.fixDeltas[i - 1].delta).toBeGreaterThanOrEqual(report.fixDeltas[i].delta);
      }
    }
  });
