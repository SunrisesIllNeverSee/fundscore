...
if (commands.has('fix')) {
  ...
  if (fixPlan.length === 0) {
    process.stdout.write('fundscore fix — nothing to scaffold. All templateable checks are passing.\n');
    process.exit(0);
  }
  ...
  if (flags.has('--apply')) {
    ...
    for (const item of fixPlan) {
      ...
      if (fs.existsSync(fullPath) && !flags.has('--force')) {
        process.stdout.write(`  ⚠️  Skipped ${item.file} (already exists, use --force to overwrite)\n`);
        continue;
      }
      const content = item.template(repoRoot);
      fs.writeFileSync(fullPath, content);
      process.stdout.write(`  ✅ Created ${item.file} (fixes: ${item.checks.join(', ')})\n`);
      written++;
    }
    ...
    for (const item of fixPlan) {
      process.stdout.write(`  📄 ${item.file} (fixes: ${item.checks.join(', ')})\n`);
    }
    process.stdout.write(`\n  Run \`fundscore fix --apply\` to create these files.\n`);
    process.stdout.write(`  Run \`fundscore fix --apply --force\` to overwrite existing files.\n`);
  }
  process.exit(0);
}
...
if (commands.has('history')) {
  ...
  if (flags.has('--save')) {
    ...
    process.stdout.write(`fundscore history — saved snapshot to ${historyFile}\n`);
    process.exit(0);
  }

  if (!fs.existsSync(historyDir)) {
    process.stdout.write('fundscore history — no history found.\n');
    process.stdout.write('  History is stored when you run fundscore with the GitHub Action or manually save reports.\n');
    process.stdout.write('  Run `fundscore` to generate a report, then `fundscore history --save` to store it.\n');
    process.exit(0);
  }
  ...
  process.stdout.write('  Date                     Score    Artifacts  Business  Quality   Round\n');
  process.stdout.write('  ───────────────────────────────────────────────────────────────────────────\n');

  for (const file of files) {
    try {
      const data = JSON.parse(fs.readFileSync(path.join(historyDir, file), 'utf8'));
      const date = data.generatedAt || file.replace('fundscore-', '').replace('.json', '');
      ...
      process.stdout.write(`  ${date}   ${overall}   ${art}   ${biz}   ${qual}   ${round}\n`);
  ...
  if (files.length >= 2) {
    try {
      const first = JSON.parse(fs.readFileSync(path.join(historyDir, files[0]), 'utf8'));
      const last = JSON.parse(fs.readFileSync(path.join(historyDir, files[files.length - 1]), 'utf8'));
      const delta = (last.scores?.overallScore ?? 0) - (first.scores?.overallScore ?? 0);
...
if (commands.has('badge')) {
  ...
  const color = s >= 70 ? '#4c1' : s >= 50 ? '#dfb317' : s >= 30 ? '#e05d44' : '#9f9f9f';
  ...
  if (flags.has('--embed')) {
    ...
    process.stdout.write(`[![fundscore](https://img.shields.io/badge/fundscore-${s.toFixed(0)}%2F100-${color.replace('#', '')})](https://github.com/SunrisesIllNeverSee/fundscore)\n`);
  } else {
