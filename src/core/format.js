...
function toMarkdown(report) {
  const { lens, lensReport, scores, status, coverage, business, quality, generatedAt, fixDeltas } = report;
  ...
  // Headline score
  lines.push(`### ${statusEmoji} Overall Score: **${scores.overallScore}/100**`);
  lines.push('');
  lines.push('> ⚠️ **Asterisk:** This is a snapshot of repo-readiness signals, not a business valuation. The score reflects what your repo communicates, not what your business is.');
  lines.push('');
  lines.push('| Dimension | Score | Weight |');
  lines.push('|-----------|-------|--------|');
  lines.push(`| Artifacts | ${scores.artifactsScore.toFixed(1)}/100 | ${Math.round(scores.weights.artifacts * 100)}% |`);
  lines.push(`| Business Viability | ${scores.businessScore.toFixed(1)}/100 | ${Math.round(scores.weights.business * 100)}% |`);
  lines.push(`| Quality | ${scores.qualityScore.toFixed(1)}/100 | ${Math.round(scores.weights.quality * 100)}% |`);
  lines.push('');
  ...
  if (lensReport) {
    lines.push(`### 📊 Round-Specific Report: ${lensReport.round} (${lensReport.roundScore}/100)`);
    lines.push('');
  ...
  lines.push('---');
  lines.push('_Powered by [fundscore](https://github.com/SunrisesIllNeverSee/fundscore) — Lighthouse for repos. Deterministic, no LLM, no external APIs._');
...
function toSummary(report) {
  ...
  if (lensReport) {
    lines.push(`  Round    : ${lensReport.round} → ${lensReport.roundScore}/100  (check size: ${lensReport.checkSize})`);
  }
